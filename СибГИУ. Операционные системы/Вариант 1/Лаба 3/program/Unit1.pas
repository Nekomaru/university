...
type
  TForm1 = class(TForm)
    ...
    Timer1: TTimer;
    Label7: TLabel;
    MemoryInfoGroupBox: TGroupBox;
    FreePartitionCountLabel: TLabel;
    EngagedPartitionsCountLabel: TLabel;
    PartitionCountLabel: TLabel;
    EngagedProcessesListBox: TListBox;
    MemoryAwaitingProcesses: TLabel;
    IODeviceInfoGroupBox: TGroupBox;
    PerformWriteToBlockLabel: TLabel;
    WritedValueLabel: TLabel;
    WriteProcessLabel: TLabel;
...
const
  PARTITION_SIZE_IN_BYTES = 32 * 1024;  //размер раздела
  MEMORY_TOTAL = 64 * 1024;             //общее кол-во памяти
  IO_DEVICE_BLOCK_COUNT = 100;
...
var
  ...
  EngagedPartitionList: TList;              //Список занятых разделов памяти
  AwaitingForMemoryProcesses: TList;        //Список процессов ожидающих память
  LastRecordedValue: integer;
  LastRecordedBlock: integer;
  IODeviceOwner: PDescriptor;               //Дескриптор процесса захватившего устройство ввода/вывода
...
function FindFirstFreePartition(var partitionIndex: Integer): boolean;
...
//функция ищет первый свободный раздел памяти
function FindFirstFreePartition(var partitionIndex: Integer): boolean;
var
  i: Integer;
begin
  if EngagedPartitionList.Count * PARTITION_SIZE_IN_BYTES = MEMORY_TOTAL then
  begin
    Result := false;
    Exit;
  end;

  partitionIndex := EngagedPartitionList.Count;
  for i := 0 to EngagedPartitionList.Count - 1 do
  begin
    if i <> Integer(EngagedPartitionList[i]^) then
    begin
      partitionIndex := i;
      break;
    end;
...
var
  ...
  NeededMemory: Integer;
  FirstFreePartitionIndex: Integer;
begin
  ...
  if OpenDialog1.Execute then begin
    ...
    Commands.LoadFromFile(OpenDialog1.FileName);
    NeededMemory := ParseInt(Commands[0]);

    if NeededMemory > PARTITION_SIZE_IN_BYTES then
    begin
      WriteToLog('Размер задачи превышает размер раздела. Задача не загружена');
      Exit;
    end;
    ...
    //пробуем найти первый свободный раздел
    if FindFirstFreePartition(FirstFreePartitionIndex) then
    begin
      //если находим делаем пометку что этот раздел занят
      EngagedPartitionList.Insert(FirstFreePartitionIndex, @FirstFreePartitionIndex);
      //сохраняем в контексте процесса индекс раздела памяти
      Process[PageControl1.ActivePageIndex].Context^.MemoryPartitionIndex := FirstFreePartitionIndex;
...
begin
  ...
  EngagedPartitionList := TList.Create;
...
begin
  ...
  //очищаем переменные хранящие номер последнего блока устройства ввода/вывода
  //в который писали данные
  LastRecordedValue := -1;
  //очищаем переменную хранящую последние записанные данные
  LastRecordedBlock := -1;
  ...
  begin
    ...
    LastRecordedValue := Random(100);
    LastRecordedBlock := Random(IO_DEVICE_BLOCK_COUNT + 1);
  end;
  ...
  begin
    ...
    //проверяем пуста ли очередь процессов ожидающих память
    if AwaitingForMemoryProcesses.Count <> 0 then
    begin
      //если не пуста то переназначаем раздел памяти на первый процесс из этой очереди
      ContextProcessByID(PDescriptor(AwaitingForMemoryProcesses[0])^.PID)^.MemoryPartitionIndex :=
        CurrentProcessContext^.MemoryPartitionIndex;
      //добавляем его в очередь готовых
      ReadyProcesses.Add(PDescriptor(AwaitingForMemoryProcesses[0]));
      //удаляем его из очереди ждущих память
      AwaitingForMemoryProcesses.Delete(0);
    end
    else
    begin
      //если пуста то помечаем, что раздел, который занимал выполнившийся раздел пустой 
      EngagedPartitionList.Delete(CurrentProcessContext^.MemoryPartitionIndex);
    end;
...
begin
  ...
  Form.EngagedProcessesListBox.Clear;
  for i := 0 to AwaitingForMemoryProcesses.Count - 1 do
  begin
    CurrentDescriptor := AwaitingForMemoryProcesses[i];
    CurrentContext := Form.ContextProcessByID(CurrentDescriptor^.PID);
    Form.EngagedProcessesListBox.Items.Add(CurrentContext^.Name);
  end;

  PartitionCount := Trunc(MEMORY_TOTAL / PARTITION_SIZE_IN_BYTES);
  Form.PartitionCountLabel.Caption := 'Всего разделов: ' + IntToStr(PartitionCount);
  Form.FreePartitionCountLabel.Caption := 'Свободных разделов: ' +
    IntToStr(PartitionCount - EngagedPartitionList.Count);
  Form.EngagedPartitionsCountLabel.Caption := 'Занятых разделов: ' + IntToStr(EngagedPartitionList.Count);

  if LastRecordedBlock = -1 then
  begin
    Form.PerformWriteToBlockLabel.Caption := 'Производится запись в блок: -';
  end else begin
    Form.PerformWriteToBlockLabel.Caption := Format('Производится запись в блок: %d', [LastRecordedBlock]);
  end;

  if LastRecordedValue = -1 then
  begin
    Form.WritedValueLabel.Caption := 'Записываемое значение: -';
  end else begin
    Form.WritedValueLabel.Caption := Format('Записываемое значение: %d', [LastRecordedValue]);
  end;

  if IODeviceOwner = nil then
  begin
    Form.WriteProcessLabel.Caption := 'Пишуший процесс: -';
  end else begin
    IODeviceOwnerContext := Form.ContextProcessByID(IODeviceOwner^.PID);
    Form.WriteProcessLabel.Caption := Format('Пишуший процесс: %s', [IODeviceOwnerContext^.Name]);
  end;
end;
