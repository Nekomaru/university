...
var
  ...
  ActiveProcessesDescriptors: TList;
...
function determineTotalUsageTimeOfiODevice(PID: integer): integer;
...
//обновляет приоритеты процессов
procedure refreshActiveProcessesPriorities;
var
  minIODeviceUsageTime, minIODeviceUsageTimeProcessIndex, minIODevicePID: integer;
  currentProcessDescriptor: PDescriptor;
  i, currentProcessIODeviceUsageTime: integer;
begin
  if ActiveProcessesDescriptors.Count = 0 then
  begin
    Exit;
  end;

  minIODeviceUsageTime := High(integer);

  for i := 0 to ActiveProcessesDescriptors.Count - 1 do
  begin
    currentProcessDescriptor := ActiveProcessesDescriptors[i];
    currentProcessDescriptor^.Prioritet := ppNormal;
    ALabel[2,currentProcessDescriptor^.PID].Caption := 'Приоритет = ' +
      PriorToStr(currentProcessDescriptor^.Prioritet);

    currentProcessIODeviceUsageTime :=
      determineTotalUsageTimeOfiODevice(currentProcessDescriptor^.PID);
    if currentProcessIODeviceUsageTime < minIODeviceUsageTime then
    begin
      minIODeviceUsageTime := currentProcessIODeviceUsageTime;
      minIODeviceUsageTimeProcessIndex := i;
      minIODevicePID := currentProcessDescriptor^.PID;
    end;
  end;
  PDescriptor(ActiveProcessesDescriptors[minIODeviceUsageTimeProcessIndex])^.Prioritet := ppHigh;
  ALabel[2,minIODevicePID].Caption := 'Приоритет = ' +
    PriorToStr(PDescriptor(ActiveProcessesDescriptors[minIODeviceUsageTimeProcessIndex])^.Prioritet);
end;

//определяет потребность процесса в устройстве ввода/вывода
function determineTotalUsageTimeOfiODevice(PID: integer): integer;
var
  processesCommandGroups: TStrings;
  totalUsage, i: integer;
  currentCommandGroupType: TCommand;
  currentCommandGroupTimeUsage: integer;
begin
  totalUsage := 0;

  processesCommandGroups := ListBox[PID].Items;
  for i := 0 to processesCommandGroups.Count - 1 do
  begin
    currentCommandGroupType := ParseCommand(processesCommandGroups[i]);
    currentCommandGroupTimeUsage := ParseInt(processesCommandGroups[i]);
    if currentCommandGroupType = cIO then
    begin
      totalUsage := totalUsage + currentCommandGroupTimeUsage;
    end;
  end;
...
begin
  ...
  if OpenDialog1.Execute then begin
    ...
    ActiveProcessesDescriptors.Add(@Process[PageControl1.PageCount-1].Descriptor);
...
begin
  ...
  ActiveProcessesDescriptors := TList.Create;
...
//проверяем выполнился ли процесс
function IsCurrentProcessExecuted: boolean;
var
